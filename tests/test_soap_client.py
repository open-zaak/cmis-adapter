...
from drc_cmis.client.exceptions import FolderDoesNotExistError, DocumentDoesNotExistError
from drc_cmis.cmis.soap_drc_document import Folder, Document
...
class CMISSOAPClientTests(DMSSOAPMixin, TestCase):
    ...
    def test_existing_folder(self):
        ...
        second_folder = self.cmis_client.create_folder("TestFolder2", base_folder.objectId)
        ...
        with self.assertRaises(FolderDoesNotExistError):
            invented_object_id = "workspace://SpacesStore/d06f86e0-1c3a-49cf-b5cd-01c079cf8147"
            self.cmis_client.get_folder(invented_object_id)

    def test_get_or_create_folder_when_folder_doesnt_exist(self):
        new_folder = self.cmis_client.get_or_create_folder(name="TestFolder", parent=self.cmis_client.base_folder)
        self.assertEqual(new_folder.name, "TestFolder")

    def test_get_or_create_folder_when_folder_exist(self):
        new_folder = self.cmis_client.create_folder(name="TestFolder", parent_id=self.cmis_client.base_folder.objectId)
        self.assertEqual(new_folder.name, "TestFolder")
        new_folder_id = new_folder.objectId

        retrieved_folder = self.cmis_client.get_or_create_folder(name="TestFolder", parent=self.cmis_client.base_folder)
    ...
    def test_create_gebruiksrechten(self):
        ...
        gebruiksrechten = self.cmis_client.create_content_object(data=properties, object_type="gebruiksrechten")

        self.assertEqual(gebruiksrechten.informatieobject, properties["informatieobject"])
        self.assertEqual(gebruiksrechten.startdatum, datetime.datetime.strptime(properties["startdatum"], "%Y-%m-%dT%H:%M:%S%z"))
        self.assertEqual(gebruiksrechten.omschrijving_voorwaarden, properties["omschrijving_voorwaarden"])
    ...
    def test_get_existing_oio(self):
        ...
        retrieved_oio = self.cmis_client.get_content_object(uuid=oio.objectId, object_type="oio")
    ...
    def test_get_existing_gebruiksrechten(self):
        ...
        retrieved_gebruiksrechten = self.cmis_client.get_content_object(uuid=gebruiksrechten.objectId, object_type="gebruiksrechten")

        self.assertEqual(gebruiksrechten.informatieobject, retrieved_gebruiksrechten.informatieobject)
        self.assertEqual(gebruiksrechten.startdatum, retrieved_gebruiksrechten.startdatum)
        self.assertEqual(gebruiksrechten.omschrijving_voorwaarden, retrieved_gebruiksrechten.omschrijving_voorwaarden)
    ...
    def test_get_non_existing_gebruiksrechten(self):
        with self.assertRaises(DocumentDoesNotExistError):
            invented_object_id = "workspace://SpacesStore/d06f86e0-1c3a-49cf-b5cd-01c079cf8147"
            self.cmis_client.get_content_object(uuid=invented_object_id, object_type="gebruiksrechten")

    def test_get_non_existing_oio(self):
        with self.assertRaises(DocumentDoesNotExistError):
            invented_object_id = "workspace://SpacesStore/d06f86e0-1c3a-49cf-b5cd-01c079cf8147"
            self.cmis_client.get_content_object(uuid=invented_object_id, object_type="oio")
    ...
    def test_delete_gebruiksrechten(self):
        gebruiksrechten = self.cmis_client.create_content_object(data={}, object_type="gebruiksrechten")
        gebruiksrechten.delete_object()
        with self.assertRaises(DocumentDoesNotExistError):
            self.cmis_client.get_content_object(uuid=gebruiksrechten.objectId, object_type="gebruiksrechten")
